...
function focusReaderMain() {
  ...
  $('script').each(function() {
    if (!this.src.match(/readability|[Cc]lippability|jquery\.min\.js$/)) {
      $(this).remove();
    }
  });
...
function focusReaderBanner() {
  ...
  articleBanner.innerHTML =
    "<span class='fleft'>" +
    document.location +
    "</span><span class='fright'><a href='http://kortina.nyc/'>kortina.nyc</a><span> Focus Reader</span></span><span style='clear:both;'></span>";
...
function grabArticle() {
  ...
  for (var j = 0; j < allParagraphs.length; j++) {
    ...
    if (getInnerText(allParagraphs[j]).length > 10) parentNode.readability.contentScore++;
  ...
  if (topDiv == null) {
    ...
    topDiv.innerHTML =
      'Focus Reader was unable to parse this page for content. Report issues <a href="https://github.com/kortina/focus-reader-view-chrome-extension/issues">here</a>.';
  }
  ...
  for (var k = 0; k < document.styleSheets.length; k++) {
    if (
      ...
      document.styleSheets[k].href.lastIndexOf('readability') == -1
    ) {
  ...
  for (var l = 0; l < styleTags.length; l++) {
    if (navigator.appName != 'Microsoft Internet Explorer') styleTags[l].textContent = '';
  }
...
function getInnerText(e) {
  if (navigator.appName == 'Microsoft Internet Explorer') return e.innerText;
...
function cleanStyles(e) {
  ...
  if (typeof e.removeAttribute == 'function') e.removeAttribute('style');
